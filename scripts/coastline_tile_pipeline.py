...
output_csv = output_root / "intersections_no_adjacent.csv"
plot_output = output_root / "coastline_tiles_plot_no_adjacent.png"
...
# === STEP 5: Label tile categories ===
coast_tiles = coast_tiles.copy()
coast_tiles["category"] = "intersecting"
other_tiles = tiles_gdf[~tiles_gdf["tile"].isin(coast_tiles["tile"])].copy()
other_tiles["category"] = "other"

all_tiles = pd.concat([coast_tiles, other_tiles])
all_tiles.to_csv(output_csv, index=False)

# === STEP 6: Generate PNGs for intersecting + adjacent tiles ===
...
# === STEP 7: Plot filled tiles with outlines ===
...
# Category outlines
colors = {"intersecting": "red", "other": "black"}
...
# PNG fill
for _, row in all_tiles[all_tiles["category"].isin(["intersecting"])].iterrows():
    png_path = png_dir / row["tile"].replace(".tif", ".png")
    if png_path.exists():
